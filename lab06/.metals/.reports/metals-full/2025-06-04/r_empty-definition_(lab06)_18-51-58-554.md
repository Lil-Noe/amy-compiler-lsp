error id: file:///C:/Users/lilia/Documents/EPFL/BA6/CLP/CS-320_Amy_LSP/lab06/src/amyc/lsp/AmyTextDocumentService.scala:`<none>`.
...
non-local guesses:
	 ...
	 -amyc/analyzer/name#
	 -amyc/analyzer/name().
	 -amyc/ast/SymbolicTreeModule.name.
	 -amyc/ast/SymbolicTreeModule.name#
	 -amyc/ast/SymbolicTreeModule.name().
	 -name.
	 -name#
...
class AmyTextDocumentService(server: AmyLanguageServer) extends TextDocumentService {
  ...
  */
  ...
  { 
      ...
      // Add the library files to name analyse without error
      val sourceDir = path.getParent()
      val libraryDir = sourceDir.resolve("library")
      val filesToLookIn = List(
        path, 
      ...
      val ctx = new Context(new Reporter, filesToLookIn.map(_.toString()))
      ...
      program.modules.foreach { mod => {
        ...
        identifier match {
          case None => {server.sendMsgToClient(s"Identifier not found")/* do nothing if the position did not point to an identifier */}
          case Some(name) => {
