error id: file:///C:/Users/lilia/Documents/EPFL/BA6/CLP/CS-320_Amy_LSP/lab06/src/amyc/parsing/Lexer.scala:silex/Source.fromFile().
file:///C:/Users/lilia/Documents/EPFL/BA6/CLP/CS-320_Amy_LSP/lab06/src/amyc/parsing/Lexer.scala
empty definition using pc, found symbol in pc: silex/Source.fromFile().
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -amyc/utils/Source.fromFile.
	 -amyc/utils/Source.fromFile#
	 -amyc/utils/Source.fromFile().
	 -silex/Source.fromFile.
	 -silex/Source.fromFile#
	 -silex/Source.fromFile().
	 -Tokens.Source.fromFile.
	 -Tokens.Source.fromFile#
	 -Tokens.Source.fromFile().
	 -Source.fromFile.
	 -Source.fromFile#
	 ...
	 -scala/Predef.Source.fromFile().
offset: 5688
uri: file:///C:/Users/lilia/Documents/EPFL/BA6/CLP/CS-320_Amy_LSP/lab06/src/amyc/parsing/Lexer.scala
text:
```scala
package amyc
package parsing
...
// The lexer for Amy.
object AmyLexer extends Pipeline[List[File], Iterator[Token]] with Lexers {
...
> { (cs, range) => KeywordToken(cs.mkString).setPos(range._1, range._2) },

    // Primitive type names,
    word("Int") | word("Boolean") | word("String") | word("Unit")
> { (cs, range) => PrimTypeToken(cs.mkString).setPos(range._1, range._2) },

    // Boolean literals,
    word("true") | word("false")
> { (cs, range) =>
        BoolLitToken(cs.mkString.toBoolean).setPos(range._1, range._2)
      },
...
> { (cs, range) => OperatorToken(cs.mkString).setPos(range._1, range._2) },

    // Identifiers,
    elem(_.isLetter) ~ many(elem(_.isLetterOrDigit) | elem('_'))
> { (cs, range) => IdentifierToken(cs.mkString).setPos(range._1, range._2) },
...
> { (cs, range) =>
        try {
          IntLitToken(cs.mkString.toInt).setPos(range._1, range._2)
        } catch {
          case e: NumberFormatException =>
            ErrorToken(cs.mkString).setPos(range._1, range._2)
        }
      },
...
> { (cs, range) =>
        StringLitToken(cs.mkString.drop(1).dropRight(1)).setPos(range._1, range._2)
      },

    // Delimiters,
    oneOf(".,:;(){}[]=") | word("=>")
> { (cs, range) => DelimiterToken(cs.mkString).setPos(range._1, range._2) },

    // Whitespace,
    many1(elem(_.isWhitespace))
> { (cs, range) => SpaceToken().setPos(range._1, range._2) },
...
> { cs => CommentToken(cs.mkString("")) },
    ...
    word("/*") ~ many(
      elem(_ != '*') | (many(elem('*')) ~ elem(c => c != '*' && c != '/'))
    )
      ~ opt(many1(elem('*')) ~ elem('/'))
> { (cs, range) =>
        if (cs.mkString.endsWith("*/")) {
          CommentToken(cs.mkString)
        } else {
          ErrorToken("Unclosed multi-line comment: '" + cs.mkString + "'")
            .setPos(range._1, range._2)
        }
      }
  ) onError {
    // We also emit ErrorTokens for Silex-handled errors.
    (cs, range) => ErrorToken(cs.mkString).setPos(range._1, range._2)
  } onEnd {
    // Once all the input has been consumed, we emit one EOFToken.
    pos => EOFToken().setPos(pos, pos)
  }
  ...
  override def run(
    ...
    for (file <- files) {
      val source = Source.fr@@omFile(file.toString, SourcePositioner(file))
      ...
      it ++= lexer
        ...
        .map {
          case token @ ErrorToken(error) =>
            ctx.reporter.fatal(
              "Unknown token at " + token.startPosition + ": " + error
            )
          case token => token
        }
    }
