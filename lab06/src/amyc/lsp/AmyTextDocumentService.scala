...
class AmyTextDocumentService(server: AmyLanguageServer) extends TextDocumentService {
  ...
  def lookForPosition(e: Expr, identifier: Identifier): Position = {
    e match {
        ...
        case Let(df, value, body) => {
          if (df.name == identifier) e.endPosition
        ...
        case Call(qname, args) => {
          ...
          args.toStream
          .map(arg => lookForPosition(arg, identifier))
          .find(_ != NoPosition)
        ...
        case Match(scrut, cases) => { 
          lookForPosition(scrut, identifier) match {
            case NoPosition => {
              ...
              cases.toStream
              .map(matchCase => lookForPosition(matchCase.expr, identifier))
              .find(_ != NoPosition)
              .getOrElse(NoPosition)
            }
            case pos => pos
  ...
  */
  ...
  def lookForIdentifier(e: Expr, positionSource: Position): Option[Identifier] = {
    ...
    e match {
        ...
        case Plus(lhs, rhs)  => { lookForIdentifier(lhs, positionSource) match {
                ...
                case None => lookForIdentifier(rhs, positionSource)
        }} 
        ...
        case Sequence(e1,e2) => { 
          lookForIdentifier(e1, positionSource) match {
                case Some(name) => Some(name)
                case None => lookForIdentifier(e2, positionSource)
        }}
        ...
        case Let(df, value, body) => {
          if (positionMatch(positionSource, df.startPosition, df.endPosition)) Some(df.name)
            ...
            lookForIdentifier(value, positionSource) match {
                case Some(name) => Some(name)
                case None => lookForIdentifier(body, positionSource)
          }}
        }
  ...
  { 
      ...
      var definitionPosition : Position = new SourcePosition(
        path.toFile(), 0, 0)
      ...
      program.modules.foreach { mod => {
        ...
        identifier match {
          ...
          case Some(name) => {
            ...
            // If not found, look in the expressions
            if (definitionPosition.line == 0 && definitionPosition.col == 0) {
              mod.optExpr.foreach { e => {
                  if (definitionPosition.line == 0 && definitionPosition.col == 0) {
