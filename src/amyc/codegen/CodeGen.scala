  ...
  def run(ctx: Context)(v: (Program, SymbolTable)): Module = {
    ...
    def cgExpr(expr: Expr)(implicit locals: Map[Identifier, Int], lh: LocalsHandler): Code = {
      expr match {
        ...
        case BooleanLiteral(b) => Comment(expr.toString) <:> (if (b) Const(1) else Const(0))
        ...
        case Neg(expr)        => Comment(expr.toString) <:> Const(0) <:> cgExpr(expr) <:> Sub
          ...
          table.getConstructor(name) match {
            ...
            case Some(constr) =>
              ...
              val allocCode: Code =
                ...
                GetGlobal(memoryBoundary) <:> adtField(args.size + 1) <:> SetGlobal(memoryBoundary)
              ...
              val fullCode: Code = args.zipWithIndex.foldLeft((allocCode <:> tagCode)) {
                case (acc, (arg, idx)) =>
                  acc <:> GetLocal(addr) <:> adtField(idx + 1) <:> cgExpr(arg) <:> Store
              }
              Comment(expr.toString) <:> fullCode <:> GetLocal(addr)

            // Function 
            case None => 
              val argsCode = args.foldLeft(Comment("args"): Code)((acc, arg) => acc <:> cgExpr(arg))
              Comment(expr.toString) <:> argsCode <:> Call(fullName(table.getFunction(name).get.owner, name))
          }
