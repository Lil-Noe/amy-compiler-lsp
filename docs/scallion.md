...
The goal of a parser is to convert the sequence of tokens generated by the lexer into an *abstract syntax tree* (AST).

In this sense, **Scallion** is a dedicated Scala parser combinators library developed by *LARA*.
This file only aims to introduce Scallion and present how to use it.
There exists an official [comprehensive API](https://epfl-lara.github.io/scallion) that is the complete reference.

The idea behind *parser combinators* is having a set of simple primitive parsers, and ways to combine them together to create more complex parsers.
