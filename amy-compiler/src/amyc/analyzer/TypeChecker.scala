      ...
      e match {
        ...
        case Variable(name) =>
          env.get(name) match {
            ...
            case None =>
              ctx.reporter.fatal(
                s"Unknown variable : $name" + e.startPosition
              )
          }
        ...
        case Call(qname, args) =>
          table.getFunction(qname) match {
            case Some(sig) =>
              ...
              val argConstraints =
                args.zip(sig.argTypes).flatMap { case (argExpr, expectedType) =>
                  genConstraints(argExpr, expectedType)
                }
      ...
      constraints match {
        ...
        case Constraint(found, expected, pos) :: more =>
          (found, expected) match {
            ...
            case _ =>
              ctx.reporter.error(
                s"Type mismatch: found $found, expected $expected",
                pos
              )
          }
      }
